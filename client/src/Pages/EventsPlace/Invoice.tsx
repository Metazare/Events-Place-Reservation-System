    ...
    const payment = () => {
        ...
        payReservation({
            ...
            amount: getReservationTotal({
                ...data?.[0],
                days:
                    (new Date(data?.[0].duration.end).getTime() -
            ...
            >
                <div className="flex flex-col gap-4 items-start ">
                        ...
                        <div className="flex gap-2 items-center color-[#303030] divide-x divide-solid flex-wrap">
                            ...
                            <div className="flex gap-1 pl-1">
                                <p className="text-[15px]">
                                    {data?.[0]?.guestCount} Guests
                                </p>
                            </div>
                            <div className="flex gap-1 pl-1">
                                <p className="text-[15px]">
                                    {data?.[0]?.eventsPlace?.location}
                                </p>
                            </div>
                            <div className="flex gap-1 pl-1">
                                <p className="text-[15px]">
                                    {data?.[0]?.eventsPlace?.placeType}
                                </p>
                            </div>
                        </div>
                    </div>
                    <a
                        href={
                            `/eventsplace/view/` +
                            data?.[0]?.eventsPlace?.eventsPlaceId
                        }
                        className="px-4 py-2 bg-[white] rounded-lg hover:bg-[#e6e6e6] cursor-pointer"
                    >
                        View Event Place
                    </a>
                    <div className="p-5 flex gap-4 items-center bg-[white] rounded-xl w-full ">
                        <Avatar
                            variant="circular"
                            src={data?.[0]?.renter?.photo}
                            alt="WEW"
                            sx={{ width: "48px", height: "48px" }}
                        />
                        <div>
                            <h6 className="text-[#141414] font-bold">
                ...
                <div>
                    <div className="bg-[white] p-4 w-full rounded-xl shadow-md">
                        <div className="flex items-start justify-between">
                            <h6 className="text-[18px] leading-[19px] font-bold ">
                                Reservation <br />
                                Details
                            </h6>
                        ...
                        <div className="flex justify-between items-center mt-8 pb-4">
                            <div className="flex gap-2 items-center">
                                ...
                                <p className="font-semibold">
                                    {moment(data?.[0]?.duration?.start).format(
                                        "MMMM Do, YYYY"
                                    )}{" "}
                                    -{" "}
                                    {moment(data?.[0]?.duration?.end).format(
                                        "MMMM Do, YYYY"
                                    )}
                                </p>
                            </div>

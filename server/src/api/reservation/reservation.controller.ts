...
import {
    logCreateReservation,
    logUpdateReservationStatus,
...
export const getReservations: RequestHandler = async (
    req: QueryRequest<GetReservation>,
    res
) => {
    ...
    if (eventsPlaceId) {
        // Find events place
        const eventsPlace = await EventsPlaceModel.exists({
            eventsPlaceId,
        }).exec();
    ...
    // Find reservations
    const reservations: ReservationPopulatedDocument[] =
        await ReservationModel.find(reservationQuery)
...
export const createReservation: RequestHandler = async (
    req: BodyRequest<CreateReservation>,
    res
) => {
    const { user, body } = req;
    if (!user) throw new Unauthorized();

    const { eventsPlaceId, amenities, startDate, days, guestCount, amount, specialRequests } =
        body;
    const checker = new CheckData();
    ...
    const amenitySearches = amenities.map(
        (amenity) =>
            new Promise(
                async (resolve: (value: ReservationAmenity | null) => void) => {
                    // Find the amenity
                    const amenityDoc: AmenityDocument | null =
                        await AmenityModel.findOne({
    ...
    const existing: ReservationDocument[] = await ReservationModel.find({
        ...
        $or: [
            { "duration.start": dateQueryRange },
            { "duration.end": dateQueryRange },
        ],
    });
    ...
    setTimeout(async () => {
        ...
        const { payment: paymentStatus, reservation: reservationStatus } =
...
export const payReservation: RequestHandler = async (
    req: BodyRequest<PayReservation>,
    res
) => {
    const { user, body } = req;
    if (!user) throw new Unauthorized();
    ...
    if (reservation.amenities.length > 0) {
        ...
        reservation.amenities.forEach((amenity, index) => {
            description += `${amenity.quantity} x ${amenity.name}`;
            if (index < reservation.amenities.length - 1) {
                description += ", ";
            }
        });
    }
...
export const cancelReservation: RequestHandler = async (
    req: BodyRequest<CancelReservation>,
    res
) => {
    const { user, body } = req;
    if (!user) throw new Unauthorized();
...
export const getReservationDates: RequestHandler = async (req, res) => {
    ...
    const reservations: ReservationDocument[] = await ReservationModel.find({
        "duration.start": { $gt: currentDate },
    });
